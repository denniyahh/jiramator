...
**Two-tier config model:** Org config (`configs/org/`) holds Jira URL, custom field IDs, sprint cadence. Team config (`configs/teams/`) holds project key, epic definitions, and ticket templates. Both are Pydantic models loaded via `load_org_config()` / `load_team_config()`.
...
- Integration tests in `test_integration.py` use the real `configs/org/marketaxess.yaml` and `configs/teams/calcs.yaml` via `scope="module"` fixtures. Exact ticket counts (2 epics, 18 per-release, 7 per-sprint = 27 total) are asserted.
